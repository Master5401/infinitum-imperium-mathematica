...
const Index = () => {
  ...
  useEffect(() => {
    const formulas = ["E = mc²", "∮ E·dℓ = -dΦB/dt", "eiπ + 1 = 0", "∇ × E = -∂B/∂t", "Principia Mathematica", "Ad Infinitum Et Ultra", "Ars Magna", "Quantum Entanglement", "∫ e^x dx = e^x + C", "lim(x→∞) (1 + 1/x)^x = e", "∑(n=1→∞) 1/n²= π²/6", "Cogito, Ergo Sum", "Ex Nihilo, Omnia", "∇ × B = μ₀J + ε₀μ₀∂E/∂t", "∇ · E = ρ/ε₀", "dx/dt = f(x,t)", "∂ψ/∂t = (-ℏ²/2m)∇²ψ", "Carpe Mathematica", "Veritas in Numeros"];
  ...
  return <div className="min-h-screen bg-[#1A1F2C] math-bg">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-2xl bg-gradient-to-r from-purple-400 to-pink-300 bg-clip-text font-extrabold text-transparent">Imperium Infinitum Mathematica</h1>
            </div>
            <div className="flex items-center space-x-4">
              <Button variant="ghost" className="text-purple-300 hover:text-purple-200 hover:bg-purple-900/20" onClick={() => navigate("/daily-challenge")}>
                <Trophy className="h-5 w-5 mr-2" />
                Daily Challenge
              </Button>
              <Button variant="ghost" className="text-purple-300 hover:text-purple-200 hover:bg-purple-900/20" onClick={() => navigate("/library")}>
                <Book className="h-5 w-5 mr-2" />
                Library
              </Button>
              <Button variant="ghost" className="text-purple-300 hover:text-purple-200 hover:bg-purple-900/20" onClick={() => navigate("/learn")}>
                <Lightbulb className="h-5 w-5 mr-2" />
                Learn
              </Button>
              <Button variant="ghost" className="text-purple-300 hover:text-purple-200 hover:bg-purple-900/20" onClick={() => navigate("/graphing")}>
                <LineChart className="h-5 w-5 mr-2" />
                Graphing
              </Button>
              <Button onClick={() => setShowSubmit(!showSubmit)} className="flex items-center space-x-2 bg-purple-900 hover:bg-purple-800 text-purple-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 bg-transparent">
        {showSubmit ? <div className="flex justify-center mb-8">
            <SubmitSequence />
            ...
            {sampleSequences.map((sequence, index) => <div key={index} className="flex justify-center">
